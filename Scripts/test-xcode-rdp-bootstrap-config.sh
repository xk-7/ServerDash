...
expect_count 1 '    preBuildScripts:' "$PROJECT_SPEC"
expect_count 1 'if ! /bin/bash "${SRCROOT}/Scripts/ensure-rdp-dependencies.sh" --verify-only' "$PROJECT_SPEC"
expect_count 1 '$(SRCROOT)/.build/rdp/ServerDashRDP.xcframework/macos-arm64_x86_64' "$PROJECT_SPEC"
expect_count 1 '          - -lServerDashRDP' "$PROJECT_SPEC"
expect_count 1 'name = "Prepare verified RDP dependencies";' "$PROJECT_FILE"
expect_count 2 '"$(SRCROOT)/.build/rdp/ServerDashRDP.xcframework/macos-arm64_x86_64",' "$PROJECT_FILE"
expect_count 2 '"-lServerDashRDP",' "$PROJECT_FILE"
