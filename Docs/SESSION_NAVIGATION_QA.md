...
日期：2026-09-07。会话录制实现后的补充回归：macOS 237/237、iPad 93/93、iPhone 93/93 通过，三端构建通过。以下保留导航修复各中间阶段记录；真实设备及人工流程仍未完成，不代表正式发布验收通过。

## 录制实现后的补充回归

- 最新 macOS 全量日志：`/tmp/serverdash-recording-verified-mac-tests.log`，原有进程取消/子进程清理测试约 0.217 秒通过，1 秒门限未放宽。
- 最新 iPad/iPhone 全量日志：`/tmp/serverdash-recording-ipad-tests.log`、`/tmp/serverdash-recording-iphone-tests.log`；通用移动端构建日志：`/tmp/serverdash-recording-mobile-build.log`。
- 最新录制功能与未执行人工项见 [录制验收记录](SESSION_RECORDING_QA.md)。本次录制代码尚未提交或推送。
...
此前全量测试启动被自动权限审核拒绝，返回额度限制提示，没有通过其他执行通道绕过。后续测试已重新获得授权，并在录制实现后完成三端全量回归；人工检查状态如下。

- [x] 最新 macOS 应用构建。
- [x] macOS 全量测试（包括新增 `SessionNavigationRegressionTests`、MRU/跨主机分屏用例）。
- [x] iPad 模拟器全量测试（包括新增迟到回调、连续重连、SFTP 子系统失败清理、目录代次、前台传输与后台取消测试）。
- [x] iPhone 模拟器全量测试。
