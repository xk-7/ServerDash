...
- RDP 开发回归（2026-09-08）已复跑 AI 加固用例：发现旧对话的面板所有权
  引用未清理，修改旧提供商地址会误撤销已切换提供商面板的授权。现已清理过期
  所有权并在安全变更通知中核对当前对话 id；原测试未放宽。独立全量 macOS
  **303/303** 通过，记录于 `/tmp/serverdash-rdp-final-mac-tests.log`。
