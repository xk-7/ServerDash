...
- 原生主侧栏、层级分组与标签目录、主机网格/Table、多选整理、隐私模式和九类设置。
- 终端统一工具栏、紧凑监控/文件检查器、15 类高亮、固定目标批执行；增加 CPU 用户态/内核态/IO 等待、连接/监听与文件句柄指标和监控过滤。
- 本地登录 Shell、IOKit/termios 串口、系统屏幕共享 VNC、SSH 高级连接选项和 HTTP-to-SOCKS 隧道。
- SFTP 精确目标上传、字面路径回归、原生多标签远程编辑、草稿恢复、版本冲突保护、目录双向预览与可选自动上传。
- V5 增量迁移与 AES-GCM WebDAV 配置同步，基线、墓碑、冲突预览和 ETag 条件提交；凭据、录制与本机授权不参与同步。
- iPhone/iPad 仅升级共享模型与能力白名单，桌面新增入口保持关闭。连接、文件和同步边界详见 `Docs/WORKBENCH_CONNECTIONS.md`、`Docs/WORKBENCH_FILES.md`、`Docs/WORKBENCH_SYNC.md`。

### Added

- 三端独立终端多标签、拖拽排序、批量关闭、快捷键与滚动标签栏；每 SSH 标签最多 16 面板、自由比例、4×4 网格与面板放大。
- 终端搜索、7 类关键词高亮、自定义正则与 12 色；命令栏及 OSC 133 集成的本地历史、命令库和片段补全。
- 移动端按会话 UUID 保存原生终端，切换保留缓冲区；关闭面板拒绝迟到连接，重挂载不重复建立 Shell。
...
- 移除 macOS 终端面板重复标题栏；关闭与放大活跃面板操作移至上方“+”菜单，原快捷键保持不变。
- 会话页使用专用服务器选择器直达终端；macOS、iPhone、iPad 的终端入口统一导航，机器详情与工作区选择不再相互覆盖。
- 普通打开按实际使用顺序复用终端，断开会话不自动重连；独立建连、关闭与重连使用会话代次隔离，迟到结果不抢页面。
- SFTP 连接、目录和传输移入工作区控制器，前台切页保留任务；关闭进行中传输需确认，移动端后台中断、下载待导出和失败连接清理按会话隔离。
- macOS 管道改为非阻塞事件驱动读取；原有取消用例定向复跑约 0.22 秒通过，保留 1 秒门限。SwiftTerm 重连更换底层进程，旧进程定向终止并回收。
...
- 最新 macOS Debug 应用构建通过（包括导航修订与标题栏精简）；移动端最终构建与三端全量回归仍待执行。
- 导航修复：中间版本 macOS/通用 iOS Simulator 构建通过，7 项进程测试与 5 项终端生命周期测试定向通过；补充测试和后续修订尚未最终构建/全量验收，权限审核返回额度限制。详见 `Docs/SESSION_NAVIGATION_QA.md`。

- 终端工作区验收：iPhone/iPad 模拟器各 84/84 项通过；macOS 新增 14 项终端测试全部通过，全量 204 项仍有既有进程取消时序用例失败（约 5.1 秒，门限 1 秒）。已检查 iPad 四面板与 iPhone 放大面板截图，实体设备和真实多主机压力测试仍待执行。
