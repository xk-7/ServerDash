...
open class TerminalView: NSView, NSTextInputClient, NSUserInterfaceValidations, TerminalDelegate {
    ...
    public var inactiveCursorStyle: InactiveCursorStyle = .outline {
        didSet {
            caretView?.inactiveStyle = inactiveCursorStyle
            caretView?.updateView()
        }
    }

    public var scrollbarVisibility: ScrollbarVisibility = .automatic {
        didSet {
    ...
    public var bellEnabled: Bool = true

    public func applyCursorStyle(_ style: CursorStyle) {
        terminal.setCursorStyle(style)
        caretView?.style = style
        updateCaretView()
    }
