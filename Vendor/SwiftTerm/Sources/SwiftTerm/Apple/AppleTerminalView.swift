...
extension TerminalView {
    ...
    func computeFontDimensions () -> CellDimension
    {
        ...
        let baseCellHeight = ceil(lineAscent + lineDescent + lineLeading)
        #if os(macOS)
        ...
        let baseCellWidth = fontSet.normal.advancement(forGlyph: glyph).width
        #else
        let fontAttributes = [NSAttributedString.Key.font: fontSet.normal]
        let baseCellWidth = "W".size(withAttributes: fontAttributes).width
        #endif
        #if os(macOS)
        let cellWidth = baseCellWidth + characterSpacing
        let cellHeight = baseCellHeight * lineHeightMultiplier
        #else
