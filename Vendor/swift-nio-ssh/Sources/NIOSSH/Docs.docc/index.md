...
Global requests are initiated using ``NIOSSHHandler/sendTCPForwardingRequest(_:promise:)`` and are received and handled by way of a ``GlobalRequestDelegate``. There are two global requests supported today:
